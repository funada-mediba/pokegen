...
function pgoapi_get(){
	...
	try {
		if (!window.openDatabase) {
            ...
            db.transaction(function(data) {
                ...
                data.executeSql('CREATE TABLE if not exists user_data(no INTEGER PRIMARY KEY AUTOINCREMENT,experience int(11) NOT NULL,km_walked int(11) NOT NULL,level int(11) NOT NULL,next_level_xp int(11) NOT NULL,poke_stop_visits int(11) NOT NULL,pokeballs_thrown int(11) NOT NULL,pokemon_caught_by_type int(11) NOT NULL,pokemons_captured int(11) NOT NULL,pokemons_encountered int(11) NOT NULL,unique_pokedex_entries int(11) NOT NULL)',[], function (data, error) {console.log(error);}, function (data, error) {console.log(error);});
            });
		}
	} catch(e) {
	    console.log("例外：" + e);
    ...
    $.ajax({
        ...
        success: function (json) {
            $(json.responses.GET_INVENTORY.inventory_delta.inventory_items).each(function(index,obj){
	            ...
	            db.transaction(function(data) {
	                for (var key in pokemon_data) {
	                    if (key == "pokemon_data") {
							...
							user_data_cnt++;
							data.executeSql('INSERT INTO user_data VALUES('+ user_data_cnt +',?,?,?,?,?,?,?,?,?,?)',[experience,km_walked,level,next_level_xp,poke_stop_visits,pokeballs_thrown,pokemon_caught_by_type,pokemons_captured,pokemons_encountered,unique_pokedex_entries]);
	                    }
	                }
				});
            });
